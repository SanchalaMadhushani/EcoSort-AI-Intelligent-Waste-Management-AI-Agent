...
| --- | --- | --- | --- |
| Waste Management | [`waste_management_agent/`](waste_management_agent/) | Single agent + function tools | [README](waste_management_agent/README.md) |

Start with `waste_management_agent/` - it shows the fundamentals (one agent, tools, an instruction prompt).

## The agents

### Waste Management - `waste_management_agent/`

A single ADK agent that answers resident and business questions about collection schedules, waste sorting & disposal guidelines, drop-off facility opening hours, and special service requests (such as bulky item pickup). Four Python function tools look up real data so the agent never guesses, `Literal` type hints constrain every tool argument to valid values, and the prompt makes the agent ask one clarifying question when a request is ambiguous - for example, asking which municipal zone a resident lives in before looking up a pickup schedule.

Try: *"Hi, I live in Zone A - North. What day is my organic waste collected?"* or *"How do I dispose of old batteries?"*

→ **[Full documentation](waste_management_agent/README.md)** - tools, data, customizing, troubleshooting.
...
│   ├── .env                # per-agent config with API key
